...
class aiXER(nn.Module):
    ...
    def forward_pre_tokenized(self, input_ids, attention_mask=None):
        # Your model forward logic using pre-tokenized inputs
        h = self.model(input_ids=input_ids, attention_mask=attention_mask).pooler_output
        return self.dense(h).sigmoid().squeeze(-1)
...
class aiXER_attention(nn.Module):
    def __init__(self, model_name:str, max_length:int=128):
        super().__init__()
        self.max_length = max_length
        self.tokenizer = AutoTokenizer.from_pretrained(model_name)
    ...
    def forward(self, x, return_attentions=False):
        ...
        if return_attentions:
            ...
            first_layer_attentions = model_outputs.attentions[0]
            # Assuming you want the attention values from the last layer
            last_layer_attentions = model_outputs.attentions[-1]
            # Average attentions across all layers
            all_layers = torch.stack(model_outputs.attentions)
            avg_attentions = torch.mean(all_layers, dim=0)
